...
async function main() {
  ...
  if (hasCredlyCols) {
    ...
    // Source CSV: First Name (0), Last Name (1), GitHub ID (2), Email (3)
    for (let i = 1; i < rows.length; i++) {
      const row = rows[i];
      ...
      // also skip empty first/last like main script does
      if (!row[0].trim() || !row[1].trim()) { skipped++; continue; }
      const key = `${email}|${templateId}`;
      if (!csvKeys.has(key)) {
        csvKeys.add(key);
        csvMap.set(key, { email, badgeTemplateId: templateId, line: i + 1 });
      }
    }
  } else {
